...
const getSinglePhoto = async (req: Request, res: Response) => {
	...
	try {
		...
		const isValidParam:boolean = await (config.crudContract.methods.photoExists as any)(Number(id)).call();
        
		if (!isValidParam) {
			return res.status(400).json({ message: "This ID Does not Exist!" });
		}
		const photo:Photo = await (config.crudContract.methods.idToPhoto as any)(Number(id)).call();
